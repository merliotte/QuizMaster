...
export function QuizQuestion() {
  const { quizState, updateQuizState, selectedTheme, settings } = useQuizStore();
  const currentQuestion = quizState.activeQuestions[quizState.currentQuestion];
  ...
  const handleNextQuestion = () => {
    if (quizState.currentQuestion < quizState.activeQuestions.length - 1) {
      updateQuizState({
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex justify-between items-center mb-6">
          <span className="text-sm font-medium text-gray-500">
            Question {quizState.currentQuestion + 1} / {quizState.activeQuestions.length}
          </span>
          <span className="text-sm font-medium text-gray-500">
        ...
        {quizState.isAnswered && quizState.currentQuestion < quizState.activeQuestions.length - 1 && (
